...
func GetTopUpInfo(c *gin.Context) {
	...
	if !paymentDisabled && isStripeTopUpEnabled() {
		// 检查是否已经包含 Stripe
		hasStripe := false
	...
	enableWaffo := !paymentDisabled && isWaffoTopUpEnabled()
	if enableWaffo {
		hasWaffo := false
		for _, method := range payMethods {
	...
	enableWaffoPancake := !paymentDisabled && isWaffoPancakeTopUpEnabled()
	...
	data := gin.H{
		...
		"enable_online_topup":        !paymentDisabled && isEpayTopUpEnabled(),
		"enable_stripe_topup":        !paymentDisabled && isStripeTopUpEnabled(),
		"enable_creem_topup":         !paymentDisabled && isCreemTopUpEnabled(),
...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		if err := model.RechargeEpay(verifyInfo.ServiceTradeNo); err != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 回调处理失败 trade_no=%s callback_type=%s client_ip=%s error=%q", verifyInfo.ServiceTradeNo, verifyInfo.Type, c.ClientIP(), err.Error()))
			return
		}
